...
def priceDetails(pageSoupTent, allTentsInfo):
    ...
    if not techPriceContainer:
        techSpecContainer = pageSoupTent.findAll("script", type="application/json")
        tentPriceText = techSpecContainer[2].string
        
        tentPriceTextSplit = tentPriceText.split("originalPrice")[1]
        startPrice = tentPriceTextSplit.find("min") + 5
        endPrice   = tentPriceTextSplit.find(",")
        curPrice = tentPriceTextSplit[startPrice:endPrice]
        
        
        startPrice = tentPriceTextSplit.find("compareAt") + 11
        endPrice   = tentPriceTextSplit.find("}")
...
allTentsUrl = "https://www.rei.com/c/backpacking-tents?ir=category%3Abackpacking-tents&r=category%3Acamping-and-hiking%7Ctents%7Cbackpacking-tents%3Bsleeping-capacity%3A3-person%3Bseasons%3A3-season%3Bbest-use%3ABackpacking%3Bdesign-type%3AFreestanding%3Bprice%3A%24200.00%20to%20%24499.99%7C%24100.00%20to%20%24199.99%7C%24500.00%20to%20%24749.99"
...
#[allTentsInfo[a][19] for a in allTentsInfo.keys()]
print("done")
#%%
[print(allTentsInfo["Floor Dimensions"][i]) for i in range(len(allTentsInfo["Tent Name"])) if allTentsInfo["Tent Name"][i].find("copper spur") != -1]

    
#%%
metricX = "Packaged Weight"
metricY = "Floor Area"
fontsize = 7

plt.plot(allTentsInfo[metricX], allTentsInfo[metricY], 'ro')

plt.xlabel(metricX)
plt.ylabel(metricY)
plt.title("Packaged Weight for Floor Area w/ Annotated Price \n for 3-Person Tents available at REI")
plt.grid("both")

for i in range(len(allTentsInfo["Tent Name"])):
    annotation = allTentsInfo["Current Price"][i]
    plt.annotate(annotation, (allTentsInfo[metricX][i], allTentsInfo[metricY][i]), fontsize = fontsize)
    
plt.savefig('tents.png', dpi = 500)
#%%
[print(allTentsInfo["Tent Name"][i]) for i in range(len(allTentsInfo["Current Price"])) if allTentsInfo["Current Price"][i] == 374.19]
[print(allTentsInfo["Original Price"][i]) for i in range(len(allTentsInfo["Current Price"])) if allTentsInfo["Current Price"][i] == 299.73]

    
#%%
from mpl_toolkits import mplot3d
ax = plt.axes(projection="3d")
ax.scatter3D(allTentsInfo["Packaged Weight"], allTentsInfo["Current Price"], allTentsInfo["Floor Area"], 'ro')
